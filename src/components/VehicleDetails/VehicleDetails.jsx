...
function VehicleFields({ form, isEditing, updateForm }) {
  ...
  const handleClientChange = (e) => {
    ...
    updateForm({ ...form, client: selectedClient });
  };
  ...
  if (!isEditing) {
    return (
      <>
        ...
        {form.client && form.client.name && (
          <div className="client-info-section">
            <h3>Cliente Asociado</h3>
            <p>
              <strong>👤 Nombre:</strong> {form.client.name}
            </p>
            <p>
              <strong>📞 Teléfono:</strong> {form.client.telephone}
            </p>
            <hr />
          </div>
        )}
        ...
        <p>
          <strong>Matrícula:</strong> {form.plate}
        </p>
        <p>
        ...
        <p>
          <strong>Kms:</strong> {form.kms ?? "N/D"}
        </p>
      </>
    );
  }
...
export default function VehicleDetails({ vehicle, onClose, onSave }) {
  ...
  return (
    <BaseCard
      ...
      onSave={onSave}
      footerContent={
        <p>Creado: {new Date(vehicle.createdAt).toLocaleDateString()}</p>
      }
    >
      <VehicleFields />
    </BaseCard>
